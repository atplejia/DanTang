        ...
        if imageLists == nil || imageLists?.count == 0 {
            ...
            if middle_image?.height != nil {
                ...
                if video_detail_info?.video_id != nil || largeImageLists?.count > 0 {
                    ...
                    cellHeight = 2 * NewMargin + titleH + imageH + 2 * kMargin + 16
                    ...
                    cellHeight = (titleH + 16 + kMargin >= imageH) ? (2 * NewMargin + titleH + kMargin + 16):(2 * NewMargin + imageH)
                }
            } else { // 没有图片,也不是视频
                titleW = SCREENW - 30
                titleH = NSString.boundingRectWithString(title!, size: CGSizeMake(titleW, CGFloat(MAXFLOAT)), fontSize: 17)
                // 没有图片，cell 的高度 = 底部间距 + 标题的高度 + 中间的间距 + 用户头像的高度 + 底部间距
                cellHeight = 2 * NewMargin + titleH + kMargin + 16
            }
        } else {
