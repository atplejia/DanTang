...
class YMScrollTitleView: UIView {
    ...
    /// 向外界传递 titles 数组
    var titlesClosure: ((titleArray: [NewsTopTitle])->())?
    /// 记录当前选中的下标
    ...
    override init(frame: CGRect) {
        ...
        // 获取首页顶部标题数据
        QJLNetworkTool.shareNetworkTool.loadNewsTitlesData { [weak self] (topTitles) in
            // 添加推荐标题
            let dict = ["category": "__all__", "name": "推荐"]
            let recommend = NewsTopTitle(dict: dict)
    ...
    func titleArrayClosure(closure: (titleArray: [NewsTopTitle])->()) {
