...
class EditAdressViewController: QJLBaseViewController {
    ...
    private func buildNavigationItem() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "保存", style: .Plain, target: self, action: #selector(saveButtonClick))
    }
    ...
    private func buildAdressView() {
        ...
        buildTextField(phoneNumberTextField!, frame: CGRectMake(x, 2 * viewHeight, textFieldWidth, viewHeight), placeholder: "信而富侠联系你的电话", tag: 2)
    ...
    private func buildUnchangedLabel(frame: CGRect, text: String) {
        ...
        label.textColor = QJLColor(50, g: 50, b: 50,a: 1)
    ...
    private func buildTextField(textField: UITextField, frame: CGRect, placeholder: String, tag: Int) {
        ...
        textField.textColor = QJLColor(50, g: 50, b: 50,a: 1)
    ...
    private func buildInputView() -> UIView {
        ...
        cancleButton.setTitleColor(QJLColor(82, g: 188, b: 248,a: 1), forState: .Normal)
        ...
        determineButton.setTitleColor(QJLColor(82, g: 188, b: 248,a: 1), forState: .Normal)
    ...
    private func buildGenderButton(button: LeftImageRightTextButton, frame: CGRect, title: String, tag: Int) {
        ...
        button.frame = frame
        button.setTitleColor(QJLColor(50, g: 50, b: 50,a: 1)
, forState: .Normal)
    ...
    func saveButtonClick() {
        if contactsTextField?.text?.characters.count <= 1 {
            SVProgressHUD.showImage(UIImage(named: "v2_orderSuccess")!, status: "我们需要你的大名~")
            return
        }
        
        if !manButton!.selected && !womenButton!.selected {
            SVProgressHUD.showImage(UIImage(named: "v2_orderSuccess")!, status: "人妖么,不男不女的~")
            return
        }
        
        if phoneNumberTextField!.text?.characters.count != 13 {
            SVProgressHUD.showImage(UIImage(named: "v2_orderSuccess")!, status: "没电话,特么怎么联系你~")
            return
        }
        
        if cityTextField?.text?.characters.count <= 0 {
            SVProgressHUD.showImage(UIImage(named: "v2_orderSuccess")!, status: "你在哪个城市啊~空空的~")
            return
        }
        
        if areaTextField?.text?.characters.count <= 2 {
            SVProgressHUD.showImage(UIImage(named: "v2_orderSuccess")!, status: "你的位置啊~")
            return
        }
        
        if adressTextField?.text?.characters.count <= 2 {
            SVProgressHUD.showImage(UIImage(named: "v2_orderSuccess")!, status: "在哪里呢啊~上哪找你去啊~")
            return
        }
