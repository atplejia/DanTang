...
class ShopCartSupermarketTableFooterView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        backView.frame = CGRectMake(0, 0, ScreenWidth, 50)
        ...
        titleLabel.frame = CGRectMake(15, 0, titleLabel.width, 50)
        addSubview(titleLabel)
        
        priceLabel.font = UIFont.systemFontOfSize(14)
        priceLabel.textColor = UIColor.redColor()
        priceLabel.frame = CGRectMake(CGRectGetMaxX(titleLabel.frame), 0, ScreenWidth * 0.5, 50)
        priceLabel.text = UserShopCarTool.sharedUserShopCar.getAllProductsPrice()
        addSubview(priceLabel)
        
        determineButton.frame = CGRectMake(ScreenWidth - 90, 0, 90, 50)
        determineButton.backgroundColor = QJLColor(253, g: 212, b: 49,a: 1)
